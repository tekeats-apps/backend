            ...
            <div class="row g-4 align-items-center">
                ...
                <div class="col-sm-auto">
                    <div class="d-flex flex-wrap align-items-start gap-2">
                        <a type="button" href="{{ route('vendor.tags.create') }}"
                            class="btn btn-success btn-label waves-effect waves-light"><i
                                class="ri-add-line label-icon align-middle fs-16 me-2"></i> Add New Tag</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-xxl-4 col-sm-12">
                    ...
                    <div class="search-box">
                        <input type="text" wire:model.debounce.500ms="search" id="search"
                            ...
                            placeholder="Search for tag name, slug, or something...">
                    ...
                    @isset($tags)
                        @foreach ($tags as $key => $tag)
                            <tr>
                                <th scope="row">
                                    <div class="form-check">
                                        <input class="form-check-input fs-15" id="category_{{ $tag->id }}"
                                            type="checkbox" value="1">
                                    </div>
                                </th>
                                <td>{{ ++$key }}</td>
                                <td>{{ $tag->name }}</td>
                                <td><span class="badge badge-soft-{{ $tag->status ? 'success' : 'danger' }}">
                                        {{ $tag->status ? 'Active' : 'Inactive' }}</span></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li><a href="{{ route('vendor.tags.edit', $tag->id) }}"
                                            ...
                                            <li>
                                                <a class="dropdown-item remove-item-btn"
                                                    wire:click="confirmDelete({{ $tag->id }})" role="button">
                                            ...
                                            <li>
                                                <a class="dropdown-item status-change-btn" role="button"
                                                    ...
                                                    <i
                                                        class="{{ $tag->status ? 'ri-arrow-down-fill text-danger' : 'ri-arrow-up-fill text-success' }} align-bottom me-2"></i>
            ...
            Livewire.on('swal:confirm-delete', function(data) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.emit('delete-tag', data.tagId);
                    }
                });
            });
        });
